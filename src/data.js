  ...
  let filtered = pokemones.filter((pokemon) => {
    if (types.length === 1 && weakness.length === 1) {
      return (
        ...
        weakness.includes(pokemon.weaknesses[0])
      );
    } else {
      return (
        types.every((e) => pokemon.type.includes(e)) &&
        weakness.every((e) => pokemon.weaknesses.includes(e))
      );
    }
  });
  return filtered;
...
export const obtenerEvoluciones = (pokemon) => {
  ...
  const evolucionesOrdenadas = prevEvolutions
    .concat(pokemon.num)
    .concat(nextEvolutions);
...
export const sortBy = (pokemonesFiltrados, ordenSeleccionado) => {
  ...
  switch (ordenSeleccionado) {
    case "Número inferior":
      resultado = pokemonesFiltrados.sort((pa, pb) => {
